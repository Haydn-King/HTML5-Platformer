...
class Hero extends Phaser.GameObjects.Sprite {
  ...
  setupAnimations() {
    ...
    this.animPredicates = {
      idle: () => {
        return (
          this.body.onFloor() && this.body.velocity.x === 0
        );
      },
      run: () => {
        return (
  ...
  preUpdate(time, delta) {
    ...
    for (const t of this.moveState.transitions()) {
      if (
        t in this.movePredicates &&
        this.movePredicates[t]()
      ) {
        this.moveState[t]();
        break;
      }
    ...
    for (const t of this.animState.transitions()) {
      if (
        t in this.animPredicates &&
        this.animPredicates[t]()
      ) {
        this.animState[t]();
        break;
      }
